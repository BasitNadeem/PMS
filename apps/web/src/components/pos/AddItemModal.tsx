...
// handleSubmit blocks the mutation unless toRecipePayload succeeds, so by the
// time this runs the draft is known good.
function toRecipeLines(recipe: RecipeDraftLine[]) {
  ...
  const mutation = useMutation({
    mutationFn: () =>
      posService.createItem(category.id, {
        ...
        isFeatured:  featured,
        ...(photoUrl ? { photoUrl } : {}),
        ingredients: toRecipeLines(recipe),
      }),
    onSuccess: () => {
